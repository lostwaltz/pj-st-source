...
public class UIEpicGachaPanel : UIBase
{
    ...
    void Start()
    {
        ...
        oneAccessIconText.text = "X30";
        tenAccessIconText.text = "X300";
        if (Core.CurrencyManager.GetAmount<Credits>() < 30)
        {
        ...
        if (Core.CurrencyManager.GetAmount<Credits>() < 300)
        {
    ...
    {
        if (Core.CurrencyManager.Spend<Credits>(30) == false)
        {
    ...
    {
        if (Core.CurrencyManager.Spend<Credits>(300) == false)
        {
            Core.UIManager.OpenUI<UIGachaWarningPanel>();
            return;
        }
