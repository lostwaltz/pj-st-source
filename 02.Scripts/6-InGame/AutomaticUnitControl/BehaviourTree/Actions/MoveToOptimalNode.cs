...
namespace UnitBT
{
    ...
    /// </summary>
    public class MoveToOptimalNode : IBehaviourNode
    {
        // 위치 평가에 사용되는 가중치
        private const float COVER_SCORE_WEIGHT = 2.0f;     // 엄폐 점수 가중치
        private const float RANGE_SCORE_WEIGHT = 1.5f;     // 사거리 점수 가중치
        private const float DISTANCE_SCORE_WEIGHT = 1.0f;  // 거리 점수 가중치
        ...
        private float EvaluatePosition(Vector2 pos, Unit subject, Unit target)
        {
            ...
            float distanceScore = EvaluateDistanceScore(pos, target.curCoord);  // 거리 점수
        ...
        private float EvaluateRangeScore(Vector2 pos, Unit subject, Unit target)
        {
            ...
            // 유닛과 타겟의 스킬 사거리 가져오기
            int subjectRange = subject.SkillSystem.ActiveSkills[0].Data.SkillRange;
            int targetRange = target.SkillSystem.ActiveSkills[0].Data.SkillRange;
        ...
        private float EvaluateDistanceScore(Vector2 pos, Vector2 targetPos)
        {
            ...
            return 1.0f / (1.0f + distance); // 거리 점수를 0~1 사이로 정규화
        }
    }
}